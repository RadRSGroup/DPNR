...
{{dueCommitments}}`,
    variables: ['confirmedSignals', 'recentSummary', 'dueCommitments', 'languageInstruction'],
    outputSchema: {
      type: 'object',
      required: ['kind', 'text'],
      properties: {
    ...
    notes:
      ...
      'never composes a card from nothing. languageInstruction (Hebrew Localization Slice E) — text is the ' +
      'card\'s real user-facing content; compose-daily-card.ts already scans the full UserProfileItem per user ' +
      'for its consent check, so preferredLanguage/genderIdentity are free to reuse, no extra read needed.',
  },
]

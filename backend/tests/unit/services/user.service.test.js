...
describe('UserService', () => {
    ...
    describe('getUserByCognitoId', () => {
        it('should return user by Cognito ID', async () => {
            const testUser = await (0, setup_1.createTestUser)({ cognitoId: 'test-cognito-456' });
    ...
    describe('getUserByEmail', () => {
        it('should return user by email', async () => {
            ...
            const testUser = await (0, setup_1.createTestUser)({ email });
    ...
    describe('updateUserRole', () => {
        it('should update user role', async () => {
            const testUser = await (0, setup_1.createTestUser)({ role: 'student' });
            const updatedUser = await userService.updateUserRole(testUser.id, 'instructor');
            expect(updatedUser.role).toBe('instructor');
        });
        it('should throw error for invalid role', async () => {
            const testUser = await (0, setup_1.createTestUser)();
            await expect(userService.updateUserRole(testUser.id, 'invalid-role'))
                .rejects.toThrow();
        });
    });
    describe('deactivateUser', () => {
        it('should deactivate user', async () => {
            const testUser = await (0, setup_1.createTestUser)({ isActive: true });
        ...
        it('should throw error for already deactivated user', async () => {
            const testUser = await (0, setup_1.createTestUser)({ isActive: false, deletedAt: new Date() });
    ...
    describe('getAllUsers', () => {
        it('should return paginated users', async () => {
            ...
            await (0, setup_1.createTestUser)({ email: 'user1@example.com', cognitoId: 'cognito-1' });
            await (0, setup_1.createTestUser)({ email: 'user2@example.com', cognitoId: 'cognito-2' });
            await (0, setup_1.createTestUser)({ email: 'user3@example.com', cognitoId: 'cognito-3' });
        ...
        it('should filter users by role', async () => {
            await (0, setup_1.createTestUser)({ email: 'student@example.com', cognitoId: 'student-1', role: 'student' });
            await (0, setup_1.createTestUser)({ email: 'instructor@example.com', cognitoId: 'instructor-1', role: 'instructor' });
            await (0, setup_1.createTestUser)({ email: 'admin@example.com', cognitoId: 'admin-1', role: 'admin' });
            const result = await userService.getAllUsers({ role: 'instructor' });
            expect(result.users).toHaveLength(1);
            expect(result.users[0].role).toBe('instructor');
        });
        it('should filter users by search term', async () => {
            await (0, setup_1.createTestUser)({ email: 'john.doe@example.com', cognitoId: 'john-1', name: 'John Doe' });
            await (0, setup_1.createTestUser)({ email: 'jane.smith@example.com', cognitoId: 'jane-1', name: 'Jane Smith' });
        ...
        it('should exclude inactive users by default', async () => {
            await (0, setup_1.createTestUser)({ email: 'active@example.com', cognitoId: 'active-1', isActive: true });
            await (0, setup_1.createTestUser)({ email: 'inactive@example.com', cognitoId: 'inactive-1', isActive: false });

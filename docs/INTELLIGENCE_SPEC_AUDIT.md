...
| 17 | Living System Behavior (interaction mode, Open Threads, Roadmap lifecycle) | ❌ (unbuilt) | Confirmed via grep: no `OpenThread` concept exists anywhere in the schema or Lambda code. No named Roadmap lifecycle states (`Emerging/Proposed/Confirmed/Active/Evolving/Paused/Archived`) exist — today's `RoadmapItem` only has a numeric `version` counter plus a single pending `RoadmapProposalItem`, which covers the *Proposed→Confirmed* transition but not the full seven-state lifecycle or the "Paused"/"Archived" states. No current-interaction-mode inference (share/be_heard/understand/explore_pattern/decide/learn/act/regulate/unknown) exists — Companion doesn't classify session intent before responding. "Do nothing is a valid recommendation" — Library's `recommendations.ts` can return an empty list, which is compatible in spirit, but there's no explicit `recommended_action: none, reason_code: integration_space` state anywhere else (e.g. Companion never explicitly tells a user "you may have enough for now"). |
...
| 28 | Privacy, Provenance & Traceability | ⚠️ | Compact encrypted summaries over raw transcripts: true (Phase 6 crypto work in progress addresses the encryption half; "no raw payloads in logs" guardrail already exists in this file's own standing rules). Traceability gap: no `TwinSignalItem` field records which prompt/model version produced it (ties to §7's finding) — can't currently answer "what prompt version generated this signal" after the fact. |

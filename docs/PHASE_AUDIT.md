...
| **4 — Slice 4: Decision Room port** | §5.3/§7: port 7-step+summary UI to `/v1`; swap AI to Bedrock | **Backend fully done (14 steps), live-seeded; frontend not started** | All 14 step files read directly in `decision-steps/`. Frontend: grepped `apps/web/src/app/decision/**` and `apps/web/src/components/decision/*.tsx` — zero references to `/v1/rooms` or the deployed API URL anywhere; all data calls still go through `lib/supabase/decisions.ts`. Matches AGENT_LOG's claim exactly. |

...
10. **Beta labeling at the Welcome/Trial moment** (§4.7, newly found Session 15 part 2): present on
    Dashboard/Account/Pricing, absent from `/signup` and the root marketing page — both Golden Path A step 1
    and the Core MVP Scope's own wording call for it at the very first moment.
11. **Real Library and Digital Twin/My Evolution Map frontend sections** (§4.7 reconnects this to Golden
    Path B step 4 directly): both backends have been real for sessions (Library since Session 6/10, Twin
    since Session 10); neither has ever had a page. Session 13's inline Library card in Companion is a
    stopgap, not a substitute.
12. **A Roadmap-revision mechanism** (§4.7, Golden Path B step 5): today's Roadmap is written once, at
    onboarding (Session 15), and never revised afterward no matter what evidence accumulates. Needs its own
    design pass (when does it revise, from which signals, "propose don't silently rewrite" framing or not)
    before building.

...
export default function Hero({
  ...
  imageSrc?: string;
  invert?: boolean;
  stacked?: boolean; // image on top, then centered text & buttons
}) {
  return (
    <section className="relative w-full bg-black text-white">
      <div className="mx-auto max-w-[1200px] px-6 py-[18vh]">
        {stacked ? (
          <>
            <Reveal>
              <div className="w-full md:w-3/4 lg:w-4/5 mx-auto rounded-xl overflow-hidden">
                <div className="relative aspect-[16/10] w-full bg-white/10">
                  {imageSrc && (
                    <Image src={imageSrc} alt="Hero" fill priority sizes="100vw" className="object-cover" />
                  )}
                </div>
              </div>
            </Reveal>
          ...
          <div className={`grid md:grid-cols-5 gap-10 items-center ${invert ? "md:[&>*:first-child]:order-2" : ""}`}>
            <Reveal className="md:col-span-2">
              <div className="text-center">
                <h1 className="font-semibold tracking-tight" style={{ fontSize: "var(--fluid-h1)" }}>{title}</h1>
                {subtitle && (
                  <p className="mt-4 text-white/80 max-w-2xl md:max-w-none mx-auto">{subtitle}</p>
                )}
                {(primary || secondary) && (
                  <div className="mt-8 flex items-center justify-center gap-4">
                    {primary && (
            ...
            <Reveal delay={100} className="md:col-span-3">
              <div className="relative aspect-[16/10] w-full rounded-xl overflow-hidden bg-white/10">
                {imageSrc && (
                  <Image src={imageSrc} alt="Hero" fill priority sizes="(min-width: 768px) 60vw, 100vw" className="object-cover" />
                )}
              </div>
            </Reveal>
          </div>
